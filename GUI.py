...
print(get_row("user_db", 1)["balance"])
...
class GUI():
	...
	def __init__(self):
		...
		self.top_three_teams_id = ez.rank("team")
		self.top_three_devs_id = ez.rank("dev")
		self.top_three_clients_id = ez.rank("client")
		...
		self.entry_reason = Entry(self.origin)
	...
	def set_up_software_stats(self):
		color = "#1A5276"
		software_statistic = [ez.total_user("dev") + ez.total_user("client"), ez.total_user("dev"), ez.total_user("client"), ez.total_user("team"),
							   ez.avg_rating(), ez.total_completed_projects(), ez.total_bids()]
		...
		label_user_total =  Label(frame, text = "Total Users: " + str(software_statistic[0]), bg = color, fg = "white")
		label_user_devs =  Label(frame, text = "Total Developers: " + str(software_statistic[1]), bg = color, fg = "white")
		label_user_clients =  Label(frame, text = "Total Clients: " + str(software_statistic[2]), bg = color, fg = "white")
		label_user_teams =  Label(frame, text = "Total Teams: " + str(software_statistic[3]), bg = color, fg = "white")
		label_avg_grade =  Label(frame, text = "Average Grade: " + str(software_statistic[4]), bg = color, fg = "white")
		label_projects_completed =  Label(frame, text = "Projects Completed: " + str(software_statistic[5]), bg = color, fg = "white")
		label_projects_completed =  Label(frame, text = "Bids Posted: " + str(software_statistic[6]), bg = color, fg = "white")
	...
	def set_up_new_team(self, event):
		...
		button_create.bind("<Button-1>", self.create_team_button)
		...
		self.entry_name_team = Entry(self.frame_profile)
		self.entry_desc_team = Text(self.frame_profile, height=2, width=30)
		...
		self.entry_name_team.grid(row = 1, column = 1, sticky = W)
		label_desc.grid(row = 2, column = 0, sticky = W)
		self.entry_desc_team.grid(row = 2, column = 1, sticky = W)
	...
	def set_up_SU_payment(self, frame_pass, Row, id):
		...
		accept_button = Button(frame, text = "Accept", bg =  "green")
		array = [issue, client, dev, log[bid["chosen_index"]][2]/2]
		accept_button.bind("<Button-1>", lambda event, arg = array: self.SU_payment_final(event, arg))
	...
	def set_up_user_profile_logged_in_info(self):
		...
		button_blacklist = Button(frame_logged_in, text = "Remove From Blacklist", bg = color, cursor = "hand2")
		...
		if self.logged_in_user["warning"] > 0 and not ez.is_on_quit_warning_list(self.logged_in_user["id"], "warning"): 
			button_dispute.config(state = NORMAL)
			button_dispute.bind("<Button-1>", self.dispute_button)
		else:
			button_dispute.config(state = DISABLED, cursor = 'arrow')
			
		if self.logged_in_user["warning"] == 2:
			button_blacklist.config(state = NORMAL)
			button_blacklist.bind("<Button-1>", self.blacklist_button)
		else:
			button_blacklist.config(state = DISABLED, cursor = 'arrow')
			
		if self.logged_in_user["balance"] <= 0:
			button_withdraw.config(state = DISABLED)
		
		if ez.is_on_quit_warning_list(self.logged_in_user["id"], "quit"):
		...
		if self.proflie_state == 0:
			...
			button_blacklist.grid(row = 3, column = 2)
	...
	def set_up_team_lists(self, frame, list, frame_num):
		...
		Index = self.project_index
		if len(list)-Index >= 3:
			amount = 3*(self.project_index/3+1)
		else:
			amount = len(list)
		while(Index < amount):
	...
	def set_up_team_profile_basic_info(self):
		...
		label_grade_static = Label(frame_basic_info, text = "Grade: " + str(ez.get_grade(self.profile_team)), bg = color)
		label_description_static = Label(frame_basic_info, text = "Description: ", bg = color)
		label_projects_static = Label(frame_basic_info, text = "Projectes Completed: " + str(len(self.profile_team["project_ids"])), bg = color)
		...
		label_active_project.grid(row = 4, column = 0, sticky = W)
		label_name_static.grid(row = 1, column = 0, sticky = W)
		label_grade_static.grid(row = 1, column = 2, sticky = W)
		label_description_static.grid(row = 2, column = 0, sticky = W)
		label_projects_static.grid(row = 3, column = 0, sticky = W)
	...
	def set_up_project_profile(self):
		...
		if self.profile_project["status"] == "bidding":
			...
			if self.logged_in_user['user_type'] == "dev"  and self.logged_in_user['status'] ==  "active":
	...
	def set_up_bids(self, frame, log):
		...
		amount = 0 
		page_amount = math.ceil((len(log)-1))
		Index = self.project_index
		if page_amount == 0:
		...
		if page_amount != 0:
			...
			while(n < (len(log)-1)):
				...
				n += 1
			if len(list_id) > 0 and Index < page_amount:
				self.make_bids((n+1), list_id[Index] ,Index,  frame)

		self.set_up_project_scroll(frame, int((self.project_index))+1, page_amount)
	
	def make_bids(self, Row, indexed_log, Index, frame):
		self.entry_reason.destroy()
		bid_frame = Frame(frame, bg = self.colors[1])
		if self.logged_in_user['id'] == self.profile_project["client_id"]:
			self.entry_reason = Entry(bid_frame)
			button_select = Button(bid_frame, text = "Pick", bg = self.colors[1])
			button_select.grid(row = 0, column = 1, sticky = E)
			self.entry_reason.grid(row = 0, column = 0, sticky = E)
			array = [self.profile_project['id'], indexed_log[1], Index+1]
		...
		label_dev_name.grid(row = 2, column = 0, sticky = W)
		label_bid_offer.grid(row = 3, column = 0, sticky = W)
		label_time_offer.grid(row = 4, column = 0, sticky = W)
		label_time_made.grid(row = 5, column = 0, sticky = W)
	...
	def set_up_profile_image(self, profile):
		...
		button_upload.bind("<Button-1>", self.upload_photo)
	...
	def set_up_project_scroll(self, frame, current_page, page_amount):    
		...
		frame_page.grid(row = page_amount*10, column = 0, columnspan = 2, sticky = E+W+S+N)
	...
	def log_out(self,event):
		...
		if self.frame_current == self.frame_profile and self.profile_type == 0:
			...
			self.set_up_top_user_chart()
		elif self.frame_current == self.frame_profile and self.profile_type == 4:
			self.set_up_top_user_chart()
	...
	def quit_button(self, event):
		Issue(self.logged_in_user['id'], "quit")
		self.set_up_profile()
		
	def dispute_button(self, event):
		Issue(self.logged_in_user['id'], "warning")
		self.set_up_profile()
		
	def dispute_button(self, event):
		Issue(self.logged_in_user['id'], "blacklisted")
		self.set_up_profile()
	...
	def create_team_button(self,event):
		if self.entry_name_team.get() != "" and self.entry_desc_team.get("1.0",END) != "":
			self.profile_team = ez.create_team(self.logged_in_user['id'], self.entry_name_team.get(), self.entry_desc_team.get("1.0",END) != "")
			self.profile_user = self.logged_in_user = get_row("user_db", self.logged_in_user['id'])
			self.set_up_team_profile()
	...
	def create_button_bid(self,event):
		...
		if self.entry_time.get() != "" and self.entry_money.get() != "" :
			...
			datetime_object = datetime_object + timedelta(days=int(self.entry_time.get()))
			if datetime_object <= datetime.strptime(self_log[0][3],"%Y-%m-%d %H:%M:%S") and float(self.entry_money.get()) <= self_log[0][2]:
				datetime_object = datetime_object.strftime("%Y-%m-%d %H:%M:%S")
				log  = ez.create_bid(self.logged_in_user['id'], self.profile_project["id"], datetime_object, float(self.entry_money.get())) #project_id, end_date, initial_bid, start_date = now
				if log != "Nan":
					bid = get_row("bid_db", self.profile_project['id'])
					bid['bid_log'] = log
					set_row("bid_db", bid)
	...
	def SU_generic_choice_accept(self, event, array):
		if self.text_reason.get("1.0",END) != "":
			if self.SU_state == 0:
			...
			array[0]["admin_review"] = self.text_reason.get("1.0",END)
	...
	def SU_payment_final(self, event, array):
		# array = [issue, client, dev, log[bid["chosen_index"]][2]/2]
		if self.entry_new_payment.get() != "" and float(self.entry_new_payment.get()) >= 0 and float(self.entry_new_payment.get()) < array[3]:
			array[1]['balance'] += array[3]-float(self.entry_new_payment.get())
			set_row("user_db", array[1])
			ez.dev_split(array[2]['id'], float(self.entry_new_payment.get()))
			array[0]['resolved'] = True
			set_row("issue_db", array[0])
		self.set_up_super_user()
	...
	def choose_bid_button(self, event, array):
		if self.entry_reason.get() != "":
			ez.choose_bid(array[0],array[1], array[2], self.entry_reason.get()) #choose_bid(bid_id, dev_id, reason = "lowest bidder"): modifies bid_db and changes states
			self.profile_project = get_row("project_db", self.profile_project['id'])
			self.profile_user = self.logged_in_user = get_row("user_db", self.logged_in_user['id'])
			self.set_up_project_profile()
	...
	def claim(self, event):
		if self.entry_grade != "Nan" and self.text_review != "Nan" and int(self.entry_grade.get()) > 0 and int(self.entry_grade.get()) < 5:
			ez.get_submission(os.path.abspath(self.path), self.profile_project['id'])
			self.profile_project = project = ez.make_rating(self.logged_in_user['id'], float(self.entry_grade.get()),self.text_review.get("1.0",END))
			set_row("project_db", project)
	...
	def upload_photo(self,event):
		self.path = fd.askopenfilename()
		ez.set_pic(os.path.dirname(self.path), os.path.basename(self.path), self.logged_in_user['id'], self.logged_in_user['user_type'])
		self.set_up_profile()
