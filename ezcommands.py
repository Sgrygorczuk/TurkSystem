...
class GUI():
    ...
    def set_up_overlay_profile(self, user, message = ""):
        ...
        label_name = Button(self.frame_logged_in, text = user["name"]+ " " + str(ezcommands.get_grade(user)), bg = color, fg = "white", cursor = "hand2")
    ...
    def set_up_new_bid(self, event):
        ...
        label_project_client_grade_static = Label(frame, text = "Grade: " + str(ezcommands.get_grade(client)), bg = "white", justify = LEFT)
    ...
    def set_up_team_search(self, Row, id):
        ...
        button_name.grid(row = 0, column = 0, padx = 40, sticky =E+W)
        label_grade = Label(frame, text = ezcommands.get_grade(team), bg = color, justify = LEFT).grid(row = 0, column = 1, padx = 40, sticky =E+W)
    ...
    def set_up_SU_payment(self, frame_pass, Row, id):
        ...
        log = ezcommands.get_bid_log(issue['referred_id'])
    ...
    def set_up_user_profile_logged_in_info(self):
        ...
        if self.proflie_state == 0:
            ...
            if self.profile_user["user_type"] == "client" and self.profile_user["status"] == "active" and (not ezcommands.is_in_active_project(self.profile_user)): #and self.profile_user["active_project_id"] = "Nan
    ...
    def set_up_project_profile(self):
        ...
        if self.profile_project["status"] == "bidding":
            ...
            if self.profile_project["status"] == "active" and dev['team_id'] == self.logged_in_user['team_id']:
                if ezcommands.is_admin(self.logged_in_user) or self.logged_in_user["id"] == dev['id']:
    ...
    def create_button_project(self,event):
        if self.entry_name.get() != "" and self.entry_desc.get("1.0",END) != ""  and  self.entry_time.get() != "" and self.entry_money.get() != ""  and float(self.entry_money.get()) < self.profile_user['balance']:
            project = ezcommands.create_project(self.logged_in_user['id'], self.entry_name.get(), self.entry_desc.get("1.0",END)) #client_id, title, desc)
            ...
            bid  = ezcommands.create_bid(project["client_id"], project["id"], datetime_object, int(self.entry_money.get())) #project_id, end_date, initial_bid, start_date = now
            if bid != "Nan":
    ...
    def create_button_bid(self,event):
        self_log = ezcommands.get_bid_log(self.profile_project['id'])
        if self.entry_time.get() != "" and self.entry_money.get() != "" :
            ...
            if datetime_object <= datetime.strptime(self_log[0][3],"%Y-%m-%d %H:%M:%S") and int(self.entry_money.get()) <= self_log[0][2]:
                datetime_object = datetime_object.strftime("%Y-%m-%d %H:%M:%S")
                log  = ezcommands.create_bid(self.logged_in_user['id'], self.profile_project["id"], datetime_object, int(self.entry_money.get())) #project_id, end_date, initial_bid, start_date = now
                if log != "Nan":
                    bid = get_row("bid_db", self.profile_project['id'])
                    bid['bid_log'] = log
                    set_row("bid_db", bid)
    ...
    def choose_bid_button(self, event, array):
        ezcommands.choose_bid(array[0],array[1]) #choose_bid(bid_id, dev_id, reason = "lowest bidder"): modifies bid_db and changes states
        self.set_up_project_profile()
    ...
    def claim(self, event):
        if self.entry_grade != "Nan" and self.text_review != "Nan" and int(self.entry_grade.get()) > 0 and int(self.entry_grade.get()) < 5:
            ezcommands.submit(os.path.dirname(self.path), os.path.basename(self.path) , self.profile_project['id'])
            project = ezcommands.make_rating(self.logged_in_user['id'], float(self.entry_grade.get()),self.text_review.get("1.0",END))
            set_row("project_db", project)
